...
imagePaths = sorted(list(paths.list_images('C:\\Users\\User\\gan_classification_many datasets\\c. GAN(a,b) + a,b')))   # data folder with 2 categorical folders
random.seed(SEED)
...
# grab the image paths and randomly shuffle them
imagePaths2 = sorted(list(paths.list_images('C:\\Users\\User\\spn_myone\\spn_pr')))   # data folder with 2 categorical folders
random.seed(SEED)
...
for train_index,test_index in KFold(n_split).split(data): #data split
    ...
    #model3.fit(trainX, trainY,epochs=15, batch_size=32)
    
    aug = ImageDataGenerator(rotation_range=10, horizontal_flip=True, vertical_flip=True, fill_mode = 'nearest')
    aug.fit(trainX)
    model3.fit_generator(aug.flow(trainX, trainY,batch_size=32), epochs=25, steps_per_epoch=len(trainX)//32)
...
#%%
prediction2 = model3.predict_classes(data3)
from sklearn.metrics import classification_report, confusion_matrix
print('Confusion Matrix')
print(confusion_matrix(labels2, prediction2))
