...
#base_model = keras.applications.nasnet.NASNetMobile(input_shape=(32, 32, 3), include_top=False, weights='imagenet', input_tensor=None, pooling=None)
...
# x = layer_dict['conv2_block3_out'].output
# x = Flatten()(x)
...
def make_model():
    ...
    print("[INFO] Compiling Model...")
    base_model = keras.applications.resnet_v2.ResNet50V2(include_top=False, weights='imagenet', input_shape=(32, 32, 3), pooling='none', classes=2)
    layer_dict = dict([(layer.name, layer) for layer in base_model.layers])
    #base_model.summary()
    plot_model(base_model, to_file='base_resnet50v2.png')                 
    for layer in base_model.layers:
        layer.trainable = False
    #for layer in base_model.layers[30:]:
        #layer.trainable = True
          
    x = layer_dict['conv4_block1_out'].output
...
for train_index,test_index in KFold(n_split).split(data): #data split
    ...
    model3.fit(trainX, trainY,epochs=35, batch_size=64, validation_data = (valx, valy))
    
    #aug = ImageDataGenerator(rotation_range=10, horizontal_flip=True, vertical_flip=True, fill_mode = 'nearest')
    #aug.fit(trainX)
    #model3.fit_generator(aug.flow(trainX, trainY,batch_size=32), epochs=18, steps_per_epoch=len(trainX)//32)
