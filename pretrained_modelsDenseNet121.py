...
# #base_model.summary()   
# #plot_model(base_model, to_file='base_dense121.png') 
       
# x = layer_dict['conv3_block1_concat'].output
...
#%%
plot_model(base_model, to_file='modeldense.png')
#%%   
...
def make_model():
    ...
    x = layer_dict['conv4_block3_concat'].output
...
for train_index,test_index in KFold(n_split).split(data): #data split
    ...
    #model3.fit(trainX, trainY,epochs=8, batch_size=32)
    
    aug = ImageDataGenerator(rotation_range=10, horizontal_flip=True, vertical_flip=True, fill_mode = 'nearest', width_shift_range=(0.1), height_shift_range=(0.1))
    aug.fit(trainX)
    model3.fit_generator(aug.flow(trainX, trainY,batch_size=32), epochs=15, steps_per_epoch=len(trainX)//32)
