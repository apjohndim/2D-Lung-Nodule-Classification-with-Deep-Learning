...
# base_model = keras.applications.xception.Xception(include_top=False, weights='imagenet', input_tensor=None, input_shape=(80,80,3), pooling=None, classes=2)
# plot_model(base_model, to_file='modelxce_default.png')                
...
# x = layer_dict['block14_sepconv2'].output
# # # x = Flatten()(x)
...
# #model.summary()
# #plot_model(model, to_file='modelxce_2.png') 

# # # plot_model(base_model, to_file='modelxce.png')
#%%   
...
def make_model(): 
    ...
    #base_model.summary()    
    layer_dict = dict([(layer.name, layer) for layer in base_model.layers])      
    x = layer_dict['block5_sepconv1'].output
    #x = Flatten()(x)
...
for imagePath in imagePaths:
    ...
    image = cv2.resize(image, (80, 80))/255
    data.append(image)
...
for imagePath2 in imagePaths2:
    ...
    image1 = cv2.resize(image1, (80, 80))/255
    data2.append(image1)
...
with tf.device('/gpu:0'):
    ...
    for train_index,test_index in KFold(n_split).split(data): #data split
        ...
        #es = EarlyStopping(monitor='val_acc',verbose=1,min_delta=0.1,patience=0,baseline=0.83)
        ...
        model3.fit(trainX, trainY, epochs=4, validation_split=0.20, batch_size=64)
